...
describe('Attribution Reporting', () => {
  ...
  it('should show Attribution Reporting content', async () => {
    await act(() => render(Attribution()));

    const attributionContent = screen.getByTestId('attribution-content');

    expect(attributionContent).toBeInTheDocument();
  });

  it('Attribution Reporting accordion should work', async () => {
    await act(() => render(Attribution()));

    const attributionContent = screen.getByTestId('attribution-content');
    const buttonElement = attributionContent?.querySelector(
      'button.flex.gap-2.text-2xl.font-bold.items-baseline.dark\\:text-bright-gray.cursor-pointer'
    );
    const attributionFrameContainer = attributionContent?.querySelector(
      'div.max-w-2xl.h-fit.flex.flex-col.gap-6.divide-y.divide-american-silver.dark\\:divide-quartz.px-4.py-6'
    );

    const attributionDescriptionRegex =
      /Allow the recording and matching of ad events with conversion events to occur on-device\..*Event-level: Determine the effectiveness of specific ad interactions to help drive optimization\..*Summary Reporting: Allows for more detail about the overall conversions \(e\.g\. region, revenue, time of day, etc\) that their advertising has delivered, while minimizing details about individual conversions\./;
    const attributionDescription = screen.getByText(
      attributionDescriptionRegex
    );
    ...
    const videoOverview = screen.getByText('Video Overview');
    const videoOverviewDescription = screen.getByText('Short summary video');
    ...
    const listDescriptionsLink: Record<string, string> = {
      ...
      'Short summary video': 'https://www.youtube.com/watch?v=UGA74CIcom8',
      'Developer documentation':
        'https://developer.chrome.com/docs/privacy-sandbox/attribution-reporting/',
    };

    expect(buttonElement?.textContent).toBe('Attribution Reporting');
    expect(attributionFrameContainer).toBeInTheDocument();
    expect(attributionFrameContainer).not.toHaveClass('hidden');

    expect(attributionDescription).toBeInTheDocument();
    expect(attributionDescription).toHaveClass(
    ...
    if (buttonElement) {
      ...
      expect(attributionFrameContainer).toHaveClass('hidden');
    }
  });
  ...
  it('should show Latest News view more button', async () => {
    ...
    const attributionContent = screen.getByTestId('attribution-content');
    const viewMoreBtn = attributionContent.querySelector(
