...
describe('Topics', () => {
  ...
  it('should show Topics content', async () => {
    await act(() => render(Topics()));

    const topicsContent = screen.getByTestId('topics-content');

    expect(topicsContent).toBeInTheDocument();
  });

  it('Topics accordion should work', async () => {
    await act(() => render(Topics()));

    const topicsContent = screen.getByTestId('topics-content');
    const buttonElement = topicsContent?.querySelector(
      'button.flex.gap-2.text-2xl.font-bold.items-baseline.dark\\:text-bright-gray.cursor-pointer'
    );
    const topicsFrameContainer = topicsContent?.querySelector(
      'div.max-w-2xl.h-fit.flex.flex-col.gap-6.divide-y.divide-american-silver.dark\\:divide-quartz.px-4.py-6'
    );

    const topicsDescriptionRegex =
    ...
    const topicsDescription = screen.getByText(topicsDescriptionRegex);
    ...
    const listDescriptionsLink: Record<string, string> = {
      ...
      'Short summary video': 'https://youtu.be/hEBzWuXjeTQ',
      'Developer documentation':
        'https://developer.chrome.com/docs/privacy-sandbox/topics/',
    };

    expect(buttonElement?.textContent).toBe('Topics');
    expect(topicsFrameContainer).toBeInTheDocument();
    expect(topicsFrameContainer).not.toHaveClass('hidden');

    expect(topicsDescription).toBeInTheDocument();
    expect(topicsDescription).toHaveClass(
    ...
    if (buttonElement) {
      ...
      expect(topicsFrameContainer).toHaveClass('hidden');
    }
  });

  it('should display topics list', async () => {
    await act(() => render(Topics()));

    const topicsListTitle = screen.getByText('Topics List');

    expect(topicsListTitle).toBeInTheDocument();
    expect(topicsListTitle).toHaveClass(
      'mb-2 text-xl font-bold tracking-tight text-gray-900 dark:text-bright-gray'
    );
  });
  ...
  it('should show Latest News view more button', async () => {
    ...
    const topicsContent = screen.getByTestId('topics-content');
    const viewMoreBtn = topicsContent.querySelector(
