...
describe('Privacy Sandbox', () => {
  ...
  it('should show Privacy Sandbox content', async () => {
    await act(() => render(PrivacySandbox()));

    const privacySandboxContent = screen.getByTestId('privacy-sandbox-content');
    const iframe = privacySandboxContent.querySelector('iframe');

    expect(privacySandboxContent).toBeInTheDocument();
    expect(iframe).toBeInTheDocument();
    expect(iframe).toHaveAttribute('src', 'https://privacysandbox.com');
    expect(iframe).toHaveClass(
      'w-full md:w-[70%] md:m-auto rounded-xl border border-hex-gray dark:border-quartz'
    );
  });

  it('Privacy Sandbox accordion should work', async () => {
    await act(() => render(PrivacySandbox()));

    const privacySandboxContent = screen.getByTestId('privacy-sandbox-content');
    const buttonElement = privacySandboxContent?.querySelector(
      'button.flex.gap-2.text-2xl.font-bold.items-baseline.dark\\:text-bright-gray.cursor-pointer'
    );
    const privacySandboxIframeContainer = privacySandboxContent?.querySelector(
      'div.h-\\[78vh\\].w-full.flex.flex-col.gap-6.divide-y.divide-american-silver.dark\\:divide-quartz.px-4.py-6'
    );

    expect(buttonElement?.textContent).toBe('Privacy Sandbox');
    expect(privacySandboxIframeContainer).toBeInTheDocument();
    expect(privacySandboxIframeContainer).not.toHaveClass('hidden');

    if (buttonElement) {
      fireEvent.click(buttonElement);

      // After button is clicked.
      expect(privacySandboxIframeContainer).toHaveClass('hidden');
    }
  });
  ...
  it('should show Latest News view more button', async () => {
    ...
    const privacySandboxContent = screen.getByTestId('privacy-sandbox-content');
    const viewMoreBtn = privacySandboxContent.querySelector(
