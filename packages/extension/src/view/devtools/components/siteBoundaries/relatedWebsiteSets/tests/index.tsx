 ...
 */
import RelatedWebsiteSets from '..';
import {
  PSInfo,
  latestNewsXML,
  latestNewsLinks,
...
describe('RelatedWebsiteSets', () => {
  ...
  it('should show RelatedWebsiteSets content', async () => {
    await act(() => render(RelatedWebsiteSets()));

    const rwsContent = screen.getByTestId('related-website-sets-content');

    expect(rwsContent).toBeInTheDocument();
  });

  it('RelatedWebsiteSets accordion should work', async () => {
    await act(() => render(RelatedWebsiteSets()));

    const rwsContent = screen.getByTestId('related-website-sets-content');
    const buttonElement = rwsContent?.querySelector(
      'button.flex.gap-2.text-2xl.font-bold.items-baseline.dark\\:text-bright-gray.cursor-pointer'
    );
    const rwsFrameContainer = rwsContent?.querySelector(
      'div.max-w-2xl.h-fit.flex.flex-col.gap-6.divide-y.divide-american-silver.dark\\:divide-quartz.px-4.py-6'
    );

    const rwsDescription = screen.getByText(
    ...
    const listDescriptionsLink: Record<string, string> = {
      ...
      'Public questions and feedback about the proposal':
        'https://github.com/WICG/first-party-sets/issues',
      'Short summary video': 'https://www.youtube.com/watch?v=cNJ8mZ-J3F8',
      'Developer documentation':
        'https://developer.chrome.com/docs/privacy-sandbox/related-website-sets/',
    };

    expect(buttonElement?.textContent).toBe('Related Website Sets');
    expect(rwsFrameContainer).toBeInTheDocument();
    expect(rwsFrameContainer).not.toHaveClass('hidden');

    expect(rwsDescription).toBeInTheDocument();
    expect(rwsDescription).toHaveClass(
    ...
    if (buttonElement) {
      ...
      expect(rwsFrameContainer).toHaveClass('hidden');
    }
  });

  it('should show RelatedWebsiteSets Membership', async () => {
    await act(() => render(RelatedWebsiteSets()));

    setTimeout(() => {
      const rwsMembership = screen.getByText('Related Website Sets Membership');
      const rwsJSONGenerator = screen.getByText(
        'Related Website Sets JSON Generator'
      );

      // It ensures that the Insights and RWSJsonGenerator component was included in RelatedWebsiteSets.
      expect(rwsMembership).toBeInTheDocument();
      expect(rwsJSONGenerator).toBeInTheDocument();
    }, 2000);
  });
  ...
  it('should show Latest News view more button', async () => {
    ...
    const rwsContent = screen.getByTestId('related-website-sets-content');
    const viewMoreBtn = rwsContent.querySelector(
