...
describe('Site Boundaries', () => {
  ...
  it('should show Site Boundaries content', async () => {
    await act(() => render(siteBoundaries()));

    const siteBoundariesContent = screen.getByTestId('site-boundaries-content');
    const iframe = siteBoundariesContent.querySelector('iframe');

    expect(siteBoundariesContent).toBeInTheDocument();
    expect(iframe).toBeInTheDocument();
    expect(iframe).toHaveAttribute(
      'src',
      'https://privacysandbox.info/en/privacy-sandbox/strengthen-privacy-boundaries'
    );
    expect(iframe).toHaveClass('w-full md:w-[70%] md:m-auto rounded-xl');
  });

  it('Site Boundaries accordion should work', async () => {
    await act(() => render(siteBoundaries()));

    const siteBoundariesContent = screen.getByTestId('site-boundaries-content');
    const buttonElement = siteBoundariesContent?.querySelector(
      'button.flex.gap-2.text-2xl.font-bold.items-baseline.dark\\:text-bright-gray.cursor-pointer'
    );
    const siteBoundariesIframeContainer = siteBoundariesContent?.querySelector(
      'div.h-\\[78vh\\].w-full.flex.flex-col.gap-6.divide-y.divide-american-silver.dark\\:divide-quartz.px-4.py-6'
    );

    expect(buttonElement?.textContent).toBe('Site Boundaries');
    expect(siteBoundariesIframeContainer).toBeInTheDocument();
    expect(siteBoundariesIframeContainer).not.toHaveClass('hidden');

    if (buttonElement) {
      fireEvent.click(buttonElement);

      // After button is clicked.
      expect(siteBoundariesIframeContainer).toHaveClass('hidden');
    }
  });
  ...
  it('should show Latest News view more button', async () => {
    ...
    const siteBoundariesContent = screen.getByTestId('site-boundaries-content');
    const viewMoreBtn = siteBoundariesContent.querySelector(
