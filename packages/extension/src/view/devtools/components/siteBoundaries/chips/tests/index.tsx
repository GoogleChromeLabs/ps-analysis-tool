...
describe('CHIPS', () => {
  ...
  it('should show CHIPS content', async () => {
    await act(() => render(Chips()));

    const chipsContent = screen.getByTestId('chips-content');

    expect(chipsContent).toBeInTheDocument();
  });

  it('CHIPS accordion should work', async () => {
    await act(() => render(Chips()));

    const chipsContent = screen.getByTestId('chips-content');
    const buttonElement = chipsContent?.querySelector(
      'button.flex.gap-2.text-2xl.font-bold.items-baseline.dark\\:text-bright-gray.cursor-pointer'
    );
    const chipsFrameContainer = chipsContent?.querySelector(
      'div.max-w-2xl.h-fit.flex.flex-col.gap-6.divide-y.divide-american-silver.dark\\:divide-quartz.px-4.py-6'
    );

    const chipsDescription = screen.getByText(
    ...
    expect(chipsFrameContainer).toBeInTheDocument();
    expect(chipsFrameContainer).not.toHaveClass('hidden');

    expect(chipsDescription).toBeInTheDocument();
    expect(chipsDescription).toHaveClass(
    ...
    if (buttonElement) {
      ...
      expect(chipsFrameContainer).toHaveClass('hidden');
    }
  });
  ...
  it('should show Latest News view more button', async () => {
    ...
    const chipsContent = screen.getByTestId('chips-content');
    const viewMoreBtn = chipsContent.querySelector(
