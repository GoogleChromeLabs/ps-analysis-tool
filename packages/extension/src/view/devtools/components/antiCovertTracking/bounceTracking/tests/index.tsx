...
describe('Bounce Tracking', () => {
  ...
  it('should show Bounce Tracking content', async () => {
    await act(() => render(BounceTracking()));

    const bounceTrackingContent = screen.getByTestId('bounce-tracking-content');

    expect(bounceTrackingContent).toBeInTheDocument();
  });

  it('Bounce Tracking accordion should work', async () => {
    await act(() => render(BounceTracking()));

    const bounceTrackingContent = screen.getByTestId('bounce-tracking-content');
    const buttonElement = bounceTrackingContent?.querySelector(
      'button.flex.gap-2.text-2xl.font-bold.items-baseline.dark\\:text-bright-gray.cursor-pointer'
    );
    const bounceTrackingFrameContainer = bounceTrackingContent?.querySelector(
      'div.max-w-2xl.h-fit.flex.flex-col.gap-6.divide-y.divide-american-silver.dark\\:divide-quartz.px-4.py-6'
    );

    const bounceTrackingDescription = screen.getByText(
    ...
    expect(buttonElement?.textContent).toBe('Bounce Tracking Mitigation');
    expect(bounceTrackingFrameContainer).toBeInTheDocument();
    expect(bounceTrackingFrameContainer).not.toHaveClass('hidden');

    expect(bounceTrackingDescription).toBeInTheDocument();
    expect(bounceTrackingDescription).toHaveClass(
    ...
    if (buttonElement) {
      ...
      expect(bounceTrackingFrameContainer).toHaveClass('hidden');
    }
  });
  ...
  it('should show Latest News view more button', async () => {
    ...
    const bounceTrackingContent = screen.getByTestId('bounce-tracking-content');
    const viewMoreBtn = bounceTrackingContent.querySelector(
