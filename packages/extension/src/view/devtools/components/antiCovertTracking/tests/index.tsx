...
describe('Tracking Protection', () => {
  ...
  it('should show Tracking Protection content', async () => {
    await act(() => render(AntiCovertTracking()));

    const trackingProtectionContent = screen.getByTestId(
      'tracking-protection-content'
    );
    const iframe = trackingProtectionContent.querySelector('iframe');

    expect(trackingProtectionContent).toBeInTheDocument();
    expect(iframe).toBeInTheDocument();
    expect(iframe).toHaveAttribute(
      'src',
      'https://privacysandbox.info/en/privacy-sandbox/prevent-covert-tracking'
    );
    expect(iframe).toHaveClass('w-full md:w-[70%] md:m-auto rounded-xl');
  });

  it('Tracking Protection accordion should work', async () => {
    await act(() => render(AntiCovertTracking()));

    const trackingProtectionContent = screen.getByTestId(
      'tracking-protection-content'
    );
    const buttonElement = trackingProtectionContent?.querySelector(
      'button.flex.gap-2.text-2xl.font-bold.items-baseline.dark\\:text-bright-gray.cursor-pointer'
    );
    const trackingProtectionIframeContainer =
      trackingProtectionContent?.querySelector(
        'div.h-\\[78vh\\].w-full.flex.flex-col.gap-6.divide-y.divide-american-silver.dark\\:divide-quartz.px-4.py-6'
      );

    expect(buttonElement?.textContent).toBe('Tracking Protection');
    expect(trackingProtectionIframeContainer).toBeInTheDocument();
    expect(trackingProtectionIframeContainer).not.toHaveClass('hidden');

    if (buttonElement) {
      fireEvent.click(buttonElement);

      // After button is clicked.
      expect(trackingProtectionIframeContainer).toHaveClass('hidden');
    }
  });
  ...
  it('should show Latest News view more button', async () => {
    ...
    const viewMoreBtn = trackingProtectionContent.querySelector(
