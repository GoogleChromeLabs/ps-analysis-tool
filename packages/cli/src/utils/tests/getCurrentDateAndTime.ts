...
describe('getCurrentDateAndTime', () => {
  beforeAll(() => {
    const mockDate = new Date(2024, 6, 16, 18, 30); // 2024-07-16 18:30
    global.Date = jest.fn(() => mockDate) as unknown as typeof Date;
  });

  it('should return date and time in YYYY-MM-DD_HH-MM format', () => {
    const result = getCurrentDateAndTime('YYYY-MM-DD_HH-MM');
    expect(result).toBe('2024-07-16_18-30');
  });

  it('should return date and time in DD MMMM, YYYY, hh:mmam/pm format', () => {
    const result = getCurrentDateAndTime('DD MMMM, YYYY, hh:mmam/pm');
    expect(result).toBe('16 July, 2024, 06:30pm');
  });
