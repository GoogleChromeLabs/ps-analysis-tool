...
const bubbles: Bubbles = {
   ...
   */
  generateBubbles: (recalculate = false) => {
   ...
   */
  barrageAnimation: async (index: number): Promise<void> => {
    const p = app.igp;
    const {
   ...
   */
  reverseBarrageAnimation: async (index: number): Promise<void> => {
    ...
    await new Promise((resolve) => {
      const animate = () => {
        ...
        const isAllBubblesReachedTargets = positionsOfCircles.every(
          (circle) => {
  ...
  bubbleChart: (
    ...
    leaf
      ...
      .attr('stroke', (d) => {
        if (
          app.bubbles.highlightedInterestGroup === titles?.[d.data as number]
        ) {
          return '#dbdb48';
        }
