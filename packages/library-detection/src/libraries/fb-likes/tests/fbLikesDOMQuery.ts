 ...
 */
import fbLikesDOMQuery from '../fbLikesDOMQuery';

describe('Facebook Likes DOM Query', () => {
  beforeEach(() => {
  ...
  it('should return the matches found', () => {
    ...
    iframe.src = 'https://www.facebook.com/v1.0/plugins/like.php?app_id=123';

    const divWithClass = document.createElement('div');
    divWithClass.classList.add('fb-like');
    ...
    const matches = fbLikesDOMQuery();

    expect(matches).toHaveLength(3);
    expect(Array.isArray(matches)).toBe(true);
    expect(matches).toContain('div[id]: fb-root');
    expect(matches).toContain('div[class]: fb-like');
