...
psat_check_chrome_update() {
  ...
  if psat_is_cache_valid "\$cache_file"; then
...
psat_launch_chrome_testing() {
  ...
  if [[ "\$os_name" == "Darwin" ]]; then
     ...
     echo "Error: Unsupported OS '\$os_name' for psat_launch_chrome_testing function." >&2
...
chrome-default() {
  ...
  psat_check_chrome_update || return $?
...
chrome-default-ps() {
  ...
  psat_check_psat_update || return $?

  psat_extension_setup || return 1 # Setup extension first
  psat_launch_chrome_testing \\
    --install-autogenerated-theme='255,51,51' \\
